...
const T: Record<string, Strings> = {
  ...
  kitabLoadError: {
    ...
    ru: "Не удалось загрузить коллекцию — обновите эту страницу.",
    ja: "コレクションを読み込めませんでした — このページを再読み込みしてください。",
  ...
  nasakhHeroDesc: {
    ...
    ru: "Собрание случаев отменяющих (насих) и отменённых (мансух) аятов в Коране, чётко упорядоченное: отменённый аят, его замена, тип, пояснение и источник. Можно прослушать.",
    ja: "クルアーンにおける廃棄（ナースィフ）と被廃棄（マンスーフ）の事例集を明快に整理：廃棄された節、その代替、種類、解説、出典。音声で聴けます。",
  ...
  nasakhIntro: {
    ...
    ru: "Наука об отменяющих (насих) и отменённых (мансух) аятах — чётко упорядоченная: что отменено, замена, тип, пояснение и источник. Можно прослушать.",
    ja: "廃棄（ナースィフ）と被廃棄（マンスーフ）の節の学 — 明快に整理：廃棄されたもの、その代替、種類、解説と出典。音声で聴けます。",
