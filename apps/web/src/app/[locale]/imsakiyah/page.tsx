import type { Metadata } from "next";
import { isValidLocale, DEFAULT_LOCALE } from "@ulyah/shared/i18n";
import { imsakiyahLabels } from "@/lib/imsakiyah-labels";
import { ImsakiyahCalendar } from "@/components/ImsakiyahCalendar";
import { localePath } from "@/lib/paths";

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  const { locale: raw } = await params;
  const locale = isValidLocale(raw) ? raw : DEFAULT_LOCALE;
  const t = imsakiyahLabels(locale);
  return {
    title: `${t.title}`,
    description: t.subtitle,
    alternates: { canonical: localePath(locale, `/imsakiyah`) },
  };
}

export default async function ImsakiyahPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale: raw } = await params;
  const locale = isValidLocale(raw) ? raw : DEFAULT_LOCALE;
  const t = imsakiyahLabels(locale);

  return (
    <div className="mx-auto max-w-xl px-4 py-14 sm:px-6">
      <h1 className="text-center font-heading text-3xl">🌙 {t.title}</h1>
      <p className="mt-2 text-center text-sm text-[var(--color-text-secondary)]">{t.subtitle}</p>

      <div className="mt-8">
        <ImsakiyahCalendar locale={locale} />
      </div>
    </div>
  );
}
