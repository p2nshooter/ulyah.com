...
const L: Record<string, { heading: string; intro: string; interactive: string; metaTitle: (s: string) => string; metaDesc: string }> = {
  ...
  ja: {
    heading: "ムスリムの子ども向け物語",
    intro:
      "動く登場人物と音声ナレーション付きの短編アニメ — 各物語がクルアーンとスンナから一つの徳を教えます。ダウンロード不要、その場で視聴。",
    interactive: "インタラクティブなアニメ物語",
    metaTitle: (s) => `ムスリムの子ども向け物語 — アニメ映像 — ${s}`,
    metaDesc:
      "子ども向けの短編アニメ：動く登場人物、音声朗読、イスラームの徳を教えます — ダウンロード不要、その場で視聴。",
  },
};
