...
function LiveHealth({ registry }: { registry: Record<string, ProviderStep[]> | null }) {
  ...
  return (
    <section className="rounded-xl border border-(--color-border) bg-(--color-card) p-4">
      ...
      {testSummary && <p className="mt-2 rounded-lg bg-black/5 p-2 text-[11px] text-text-secondary">{testSummary}</p>}
      {err && <p className="mt-2 text-xs text-danger">Gagal memuat status (butuh login admin).</p>}
      {!err && rows === null && <p className="mt-2 text-xs text-text-secondary">Memuat…</p>}
        ...
        <>
          ...
          <div className="mt-3 overflow-x-auto">
            <table className="w-full min-w-[420px] text-[11px]">
              <thead>
                <tr className="border-b border-(--color-border) text-left text-text-secondary">
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={i} className="border-b border-(--color-border) last:border-0">
                    ...
                    <td className="px-2 py-1 text-text-secondary">{r.scope}</td>
              ...
              <div key={cap} className="flex flex-wrap items-center gap-1.5 rounded-lg bg-black/5 px-2.5 py-1.5 text-[11px]">
                ...
                {steps.map((s, i) => (
                  <span key={i} className="flex items-center gap-1">
                    <span className={`rounded-sm px-1.5 py-0.5 ${activeProviders.has(s.provider) ? "bg-success/15 text-success" : "text-text-secondary/60"}`}>
                      {s.provider}
                    </span>
  ...
  return (
    <section className="rounded-xl border border-(--color-border) bg-(--color-card) p-4">
      ...
      <p className="mt-1 text-[11px] leading-relaxed text-text-secondary">
        Jalankan <code className="rounded-sm bg-black/10 px-1">scripts/kaggle-orchestra-server.py</code> di notebook Kaggle
      ...
      {cfg?.configured && (
        <p className="mt-2 text-[11px]">
          Status:{" "}
          <span className={cfg.enabled ? "text-success" : "text-text-secondary"}>
      ...
      <div className="mt-3 space-y-2">
        <input
          ...
          className="w-full rounded-lg border border-(--color-border) bg-transparent px-3 py-2 text-xs"
        />
        <div className="flex flex-wrap gap-2">
          <input
            ...
            className="min-w-[180px] flex-1 rounded-lg border border-(--color-border) bg-transparent px-3 py-2 text-xs"
          />
          <input
            value={token}
      ...
      <div className="mt-3 flex flex-wrap gap-2 text-[11px]">
        ...
        <button onClick={() => save(false)} disabled={busy} className="rounded-full border border-(--color-border) px-3 py-1.5 hover:border-accent">
          Nonaktifkan
        </button>
      ...
      {msg && <p className="mt-2 rounded-lg bg-black/5 p-2 text-[11px] text-text-secondary">{msg}</p>}
    </section>
  );
}
...
function WorkerRegistry() {
  ...
  return (
    <section className="rounded-xl border border-(--color-border) bg-(--color-card) p-4">
      ...
      {!err && workers === null && <p className="mt-2 text-xs text-text-secondary">Memuat…</p>}
      {workers !== null && (
        <div className="mt-3 overflow-x-auto rounded-xl border border-(--color-border)">
          <table className="w-full min-w-[520px] text-xs">
            <thead>
              <tr className="border-b border-(--color-border) bg-black/5">
            ...
            <tbody>
              {workers.map((w) => (
                <tr key={w.name} className="border-b border-(--color-border) last:border-0">
                  <td className="px-3 py-2 font-medium">{w.label}</td>
                  <td className="px-3 py-2 text-text-secondary">{w.desc}</td>
                  <td className="px-3 py-2"><code className="rounded-sm bg-black/10 px-1.5 py-0.5">{w.capability}</code></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {data && (
        <>
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {data.byStatus.map((s) => (
              <span key={s.status} className="rounded-full border border-(--color-border) px-3 py-1 text-[11px]">
...
function ContentBotPanel() {
  ...
  return (
    <section className="rounded-xl border border-(--color-border) bg-(--color-card) p-4">
      ...
      {!cfg?.tokenConfigured && (
        <p className="mt-2 rounded-lg bg-warning/10 p-2 text-[11px] leading-relaxed text-text-secondary">
          Untuk mengaktifkan: tambahkan secret <code className="rounded-sm bg-black/10 px-1">GH_CONTENT_TOKEN</code> (GitHub
      ...
      <div className="mt-3 flex flex-wrap gap-2 text-[11px]">
        ...
        {cfg?.paused ? (
          ...
          <button onClick={() => toggle(true)} disabled={busy} className="rounded-full border border-(--color-border) px-3 py-1.5 hover:border-danger disabled:opacity-50">
            ⏸ Jeda
          </button>
        )}
      </div>
      ...
      {(msg || cfg?.lastRun) && (
        <p className="mt-2 rounded-lg bg-black/5 p-2 text-[11px] text-text-secondary">
