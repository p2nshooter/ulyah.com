  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-(--color-border) bg-(--color-card) p-5">
        ...
        <p className="mt-1 text-sm text-text-secondary">{t.maalIntro}</p>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <label className="text-xs">
            <span className="text-text-secondary">{t.pricePerGramLabel}</span>
            <input
              inputMode="decimal"
              value={pricePerGram}
          ...
          <label className="text-xs">
            <span className="text-text-secondary">{t.wealthLabel}</span>
            <input
              inputMode="decimal"
              value={wealth}
        ...
        <div className="mt-4 rounded-xl bg-black/5 p-4 text-sm">
          <div className="flex items-center justify-between">
            <span className="text-text-secondary">{t.nisabValueLabel}</span>
          ...
          {nisabValue > 0 && (
            <div className="mt-2 border-t border-(--color-border) pt-2">
              {meetsNisab ? (
                <div className="flex items-center justify-between">
                  <span className="text-text-secondary">{t.zakatDueLabel}</span>
                  <span className="font-heading text-lg text-accent tabular-nums">{fmt(zakatMaalDue)}</span>
                </div>
              ) : (
                <p className="text-xs text-text-secondary">{t.belowNisab}</p>
              )}
            </div>
          )}
        </div>
      ...
      <section className="rounded-2xl border border-(--color-border) bg-(--color-card) p-5">
        ...
        <p className="mt-1 text-sm text-text-secondary">{t.fitrahIntro}</p>

        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <label className="text-xs">
            <span className="text-text-secondary">{t.peopleLabel}</span>
            <input
              inputMode="numeric"
              value={people}
          ...
          <label className="text-xs">
            <span className="text-text-secondary">{t.staplePriceLabel}</span>
            <input
              inputMode="decimal"
              value={staplePrice}
        ...
        <div className="mt-4 flex items-center justify-between rounded-xl bg-black/5 p-4 text-sm">
          <span className="text-text-secondary">{t.fitrahTotalLabel}</span>
