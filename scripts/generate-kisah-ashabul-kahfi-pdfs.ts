/**
 * Renders one real PDF per (episode, language) from
 * scripts/content/kisah-ashabul-kahfi.ts into
 * packages/db-schema/seed/assets/kisah-ashabul-kahfi/<lang>/<slug>.pdf — the
 * exact r2_key paths referenced by packages/db-schema/seed/kisah_dzulqarnain.sql.
 *
 * Run: npx tsx scripts/generate-kisah-ashabul-kahfi-pdfs.ts
 */
import { createWriteStream, mkdirSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
import PDFDocument from "pdfkit";
import { KISAH_ASHABUL_KAHFI_SERIES } from "./content/kisah-ashabul-kahfi.js";

const __dirname = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(__dirname, "..");
const outRoot = join(repoRoot, "packages", "db-schema", "seed", "assets", "kisah-ashabul-kahfi");
...
function renderPdf(filePath: string, title: string, subtitle: string, body: string, footer: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    doc.fontSize(9).fillColor("#B8892B").text("ULYAH.COM — Kisah Ashabul Kahfi", { align: "center" });
    doc.moveDown(1.2);
    doc.fontSize(17).fillColor("#0B3D2E").text(title, { align: "center" });
    doc.moveDown(0.3);
...
async function main() {
  ...
  for (const ep of KISAH_ASHABUL_KAHFI_SERIES) {
    for (const lang of ["id", "en"] as const) {
      ...
      const subtitle =
        lang === "id"
          ? `Berdasarkan QS. Al-Kahf (18) : ${ep.ayahStart}-${ep.ayahEnd}`
          : `Based on Qur'an, Surah Al-Kahf (18) : ${ep.ayahStart}-${ep.ayahEnd}`;
      const footer =
        lang === "id"
