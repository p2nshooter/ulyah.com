 ...
 * scripts/content/kisah-nuh.ts into
 * packages/db-schema/seed/assets/kisah-nuh/<lang>/<slug>.pdf — the exact
 * r2_key paths referenced by packages/db-schema/seed/kisah_nuh.sql.
 *
 * Run: npx tsx scripts/generate-kisah-nuh-pdfs.ts
 */
...
const outRoot = join(repoRoot, "packages", "db-schema", "seed", "assets", "kisah-nuh");
...
function renderPdf(filePath: string, title: string, subtitle: string, body: string, footer: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    doc.fontSize(9).fillColor("#B8892B").text("ULYAH.COM — Kisah Nabi Nuh AS", { align: "center" });
    doc.moveDown(1.2);
...
async function main() {
  ...
  for (const ep of KISAH_NUH_SERIES) {
    ...
    for (const lang of ["id", "en"] as const) {
      ...
      const subtitle =
        lang === "id"
          ? `Berdasarkan QS. ${surahLabel} (${ep.surahId}) : ${ep.ayahStart}-${ep.ayahEnd}`
          : `Based on Qur'an, Surah ${surahLabel} (${ep.surahId}) : ${ep.ayahStart}-${ep.ayahEnd}`;
      const footer =
        lang === "id"
