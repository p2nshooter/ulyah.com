/**
 * Renders one real PDF per (episode, language) from scripts/content/kisah-musa.ts
 * into packages/db-schema/seed/assets/kisah-nabi-musa/<lang>/<slug>.pdf — the
 * exact r2_key paths referenced by packages/db-schema/seed/kisah_musa.sql.
 *
 ...
 * rows this script's sibling (generate-kisah-musa-seed.ts) inserts always resolve
 * to an actual downloadable file, never a dangling reference.
 *
 * Run: npx tsx scripts/generate-kisah-musa-pdfs.ts
 */
...
const outRoot = join(repoRoot, "packages", "db-schema", "seed", "assets", "kisah-nabi-musa");
...
function renderPdf(filePath: string, title: string, subtitle: string, body: string, footer: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    doc.fontSize(9).fillColor("#B8892B").text("ULYAH.COM — Kisah Nabi Musa AS", { align: "center" });
    doc.moveDown(1.2);
...
async function main() {
  ...
  for (const ep of KISAH_MUSA_SERIES) {
    for (const lang of ["id", "en"] as const) {
      ...
      const surahLabel = ep.surahId === 28 ? "Al-Qasas (28)" : ep.surahId === 26 ? "Asy-Syu'ara (26)" : `Surah ${ep.surahId}`;
      const surahLabelEn = ep.surahId === 28 ? "Al-Qasas (28)" : ep.surahId === 26 ? "Ash-Shu'ara (26)" : `Surah ${ep.surahId}`;
      const subtitle =
        lang === "id"
          ? `Berdasarkan QS. ${surahLabel} : ${ep.ayahStart}-${ep.ayahEnd}`
          : `Based on Qur'an, Surah ${surahLabelEn} : ${ep.ayahStart}-${ep.ayahEnd}`;
      const footer =
        lang === "id"
