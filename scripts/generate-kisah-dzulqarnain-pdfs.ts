 ...
 * scripts/content/kisah-dzulqarnain.ts into
 * packages/db-schema/seed/assets/kisah-dzulqarnain/<lang>/<slug>.pdf — the
 * exact r2_key paths referenced by packages/db-schema/seed/kisah_dzulqarnain.sql.
 *
 * Run: npx tsx scripts/generate-kisah-dzulqarnain-pdfs.ts
 */
import { createWriteStream, mkdirSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
import PDFDocument from "pdfkit";
import { KISAH_DZULQARNAIN_SERIES } from "./content/kisah-dzulqarnain.js";

const __dirname = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(__dirname, "..");
const outRoot = join(repoRoot, "packages", "db-schema", "seed", "assets", "kisah-dzulqarnain");
...
function renderPdf(filePath: string, title: string, subtitle: string, body: string, footer: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    doc.fontSize(9).fillColor("#B8892B").text("ULYAH.COM — Kisah Dzulqarnain", { align: "center" });
    doc.moveDown(1.2);
