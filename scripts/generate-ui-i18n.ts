...
import { isLocaleReady } from "../packages/shared/src/i18n.js";
...
// The COMPLETE table, for tooling only — never imported by app code, so it
// never reaches the Worker bundle. scripts/generate-locale-readiness.ts reads
// this to score every language, including the ones we do not ship yet.
const OUT_FULL = resolve(__dirname, "ui-i18n-full.json");
...
async function main() {
  ...
  // 3a) The complete table goes to JSON, for the readiness scorer only.
  writeFileSync(OUT_FULL, JSON.stringify(table));

  // 3b) The BUNDLED module carries only the languages the sites actually serve.
  //
  // This module is imported by every page and by the middleware, so its size is
  // the Worker's size. Shipping all 26 languages made it 2.3 MB (813 KB gzipped)
  // and pushed the Worker past Cloudflare's 3 MiB limit — the deploy failed
  // outright. Since a language that has not reached 100% is redirected to the
  // site's own language and never rendered, its translations are dead weight in
  // the bundle. They come back automatically the moment the language unlocks,
  // because this filter reads the same measured readiness the gate does.
  const shipped: Record<string, Record<string, string>> = {};
  for (const tl of TARGET_LOCALES) {
    if (!isLocaleReady(tl)) continue;
    shipped[tl] = table[tl]!;
  }
  ...
  const banner =
    ...
    "// the languages we serve localizes 100% of the menus/buttons, never a mix.\n" +
    "//\n" +
    "// Only languages that are actually SERVED are included. This module ships in\n" +
    "// the Worker bundle, and Cloudflare caps a Worker at 3 MiB; carrying all 26\n" +
    "// languages (including the 22 that redirect to the site's own language and\n" +
    "// are never rendered) blew that limit and broke the deploy. The complete\n" +
    "// table lives in scripts/ui-i18n-full.json for the readiness scorer.\n" +
    "// Re-run after changing UI strings:  pnpm gen:ui-i18n\n\n";
  const body = `export const UI_I18N: Record<string, Record<string, string>> = ${JSON.stringify(shipped, null, 0)};\n`;
  writeFileSync(OUT, banner + body);
  const totalAll = Object.values(table).reduce((n, m) => n + Object.keys(m).length, 0);
  const totalShipped = Object.values(shipped).reduce((n, m) => n + Object.keys(m).length, 0);
  console.log(
    `Measured ${TARGET_LOCALES.length} locales / ${totalAll} entries -> ${OUT_FULL}\n` +
      `Shipped ${Object.keys(shipped).length} served locales (${Object.keys(shipped).join(", ")}) / ${totalShipped} entries -> ${OUT}`
  );
}
